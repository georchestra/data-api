...
class PostgisFormat extends GeoToolsFormat {

    private final @NonNull PostgisFormatProperties defaults;

    private final @Getter String name = "PostGIS";
    ...
    public boolean canHandle(@NonNull URI datasetUri) {
        ...
        Map<String, String> queryParams = URLEncodedUtils.parse(datasetUri, StandardCharsets.UTF_8).stream()
    ...
    protected Map<String, ?> toConnectionParams(@NonNull DataQuery query) {
        ...
        if (!canHandle(source.getUri())) {
            throw new IllegalArgumentException("Query URI not recognized as a WFS GetCapabilities request");
        }
